...
func ToJSON(theData interface{}, jsonIter_ ...*jsoniter.API) (resultStr string, err error) {
	...
	switch theData.(type) {
	case *cypressutils.CypressArrayList:
		{
			if theData == nil {
				return "[]", nil
			}
			return marshalCypressList(jsonIter, theData.(*cypressutils.CypressArrayList))
		}
	case *cypressutils.CypressHashMap:
		{
			if theData == nil {
				return "{}", nil
			}
			return marshalCypressHashMap(jsonIter, theData.(*cypressutils.CypressHashMap))
		}
	case *cypressutils.PageableWrapper, *cypressutils.TransactionWrapper:
		{
...
func marshalCypressList(jsonIter *jsoniter.API, theList *cypressutils.CypressArrayList) (resultStr string, err error) {
	var result []byte
	result = append(result, '[')
...
func marshalCypressHashMap(jsonIter *jsoniter.API, theHashMap *cypressutils.CypressHashMap) (resultStr string, err error) {
	...
	for pair := theHashMap.GetData().Oldest(); pair != nil; pair = pair.Next() {
		...
		case *cypressutils.CypressHashMap:
			{
				tempResultStr, err = marshalCypressHashMap(jsonIter, theValue.(*cypressutils.CypressHashMap))
				if err != nil {
					exceptions.ThrowException(cErrors.Cause(err))
					return "", err
				}
		...
		case *cypressutils.CypressArrayList:
			{
				tempResultStr, err = marshalCypressList(jsonIter, theValue.(*cypressutils.CypressArrayList))
				if err != nil {
					exceptions.ThrowException(cErrors.Cause(err))
					return "", err
				}
...
func marshalComplexObject(jsonIter *jsoniter.API, s interface{}) (resultStr string, err error) {
	...
	for index := range fields {
		...
		case *cypressutils.CypressHashMap, *cypressutils.CypressArrayList, *cypressutils.PageableWrapper, *cypressutils.TransactionWrapper:
			{
...
func parseobject(hashmap *cypressutils.CypressHashMap, dec *json.Decoder) (err error) {
	var t json.Token
	for dec.More() {
		t, err = dec.Token()
		if err != nil {
...
func parsearray(dec *json.Decoder) (result interface{}, err error) {
	...
	arrayList := cypressutils.NewList()
	...
	for dec.More() {
		...
		case *cypressutils.CypressHashMap:
			{
				arrayList.AddNewRecord(value.(*cypressutils.CypressHashMap))
				shouldReturnList = true
				shouldReturnSlice = false
			}
		case *cypressutils.CypressArrayList:
			{
				arrayList = value.(*cypressutils.CypressArrayList)
				shouldReturnList = true
				shouldReturnSlice = false
			}
		default:
