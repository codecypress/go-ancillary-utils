...
var var_BEGIN_CLAUSES = cypressutils.NewSet()
var var_END_CLAUSES = cypressutils.NewSet()
var var_LOGICAL = cypressutils.NewSet()
var var_QUANTIFIERS = cypressutils.NewSet()
var var_DML = cypressutils.NewSet()
var var_MISC = cypressutils.NewSet()
...
type formatProcess struct {
	...
	result bytes.Buffer
	tokens *miscellaneous.StringTokenizer
...
func newFormatProcess(sql string) *formatProcess {
	return &formatProcess{
		beginLine:              true,
		indent:                 1,
		tokens:                 miscellaneous.NewStringTokenizer(sql, "()+*/-=<>'`\"[],"+const_WHITESPACE, true),
		parenCounts:            []int{},
		afterByOrFromOrSelects: []bool{},
	}
}
