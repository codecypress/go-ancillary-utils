...
func NewNamedParameterQuery(sqlQuery string, queryArguments *cypressutils.CypressHashMap) *NamedParameterQuery {
...
func (namedParameterQuery *NamedParameterQuery) parseSQLQuery() {
	...
	for i := 0; i < length; i++ {
		...
		} else if c == ':' && i+1 < length && miscellaneous.IsIdentifierStart(namedParameterQuery.originalQuery[i+1]) &&
			(i-1 >= 0 && namedParameterQuery.originalQuery[i-1] != ':') {

			j := i + 2
			for j < length && miscellaneous.IsIdentifierPart(namedParameterQuery.originalQuery[j]) {
				j++
			}
